...
import { ServerRequest } from './serverRequest_refactor.js';
...
const apiResponse = new ServerRequest();

async function randomMoviePicker(n) {
  const array = await apiResponse.getRandomMovieS(n);
  const result = array.map(x => {
    ...
    const link = makeTag('a', 'poster-link', {
      href: `https://www.google.com/search?q=${x.movieTitle.replace(
        /[\s]/g,
        '+',
      )}+watch+online`,
      target: 'blank',
...
async function addMovies(placement, quantity) {
  const x = await randomMoviePicker(quantity);
