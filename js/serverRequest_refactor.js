...
export class ServerRequest {
  ...
  async getMovieData(id) {
    ...
    const options = {
      method: 'GET',
      headers: {
        'X-RapidAPI-Key': ServerRequest.apiKey,
        'X-RapidAPI-Host': ServerRequest.apiHost,
      },
    };
    try {
      const response = await fetch(url, options);
  ...
  async getRandomMovie() {
    const randomIndex = Math.floor((this.data.length - 1) * Math.random());
    const movieId = this.data[randomIndex];
    console.log(movieId, randomIndex);
    const result = await this.getMovieData(movieId); // object {}
    this.data.splice(randomIndex, 1);
    return result;
  }

  async getRandomMovieS(n) {
    let resultArr = [];
    for (let i = 0; i < n; i++) {
      resultArr.push(this.getRandomMovie());
    }
  ...
  transformMovie(movie) {
    console.log(movie);
    const imageSrc =
      movie?.contributionQuestions?.edges[0]?.node?.entity?.primaryImage?.url ||
      '#';
    const movieTitle = movie.originalTitleText.text;
    return { imageSrc, movieTitle };
  }
}
